...
public class Pagination<T>
{
	...
	public void removeElement(final int index)
	{
		...
		if (index >= this.elements.size()) throw new IllegalArgumentException(String.format("Index : %d Size: %d", index, this.countElements()));
	...
	public boolean hasNext(final Page page)
	{
		...
		if (this.contains(page)) throw new IllegalArgumentException("Page does not belong to the pagination.");
	...
	public Page getPage(final int number)
	{
		...
		if (number < 1 || number > this.pages.size()) throw new IllegalArgumentException(String.format("Page number must be between 1 and %d (include).", this.pages.size()));
