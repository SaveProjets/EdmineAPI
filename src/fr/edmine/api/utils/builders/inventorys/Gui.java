...
public abstract class Gui<P extends JavaPlugin>
{
	...
	{
		IntStream.range(0, getSize()).filter(i -> getItem(i) != null).forEach(i -> setItem(i, button));
	}
	...
	{
		IntStream.range(minimumSlot, getSize()).filter(i -> getItem(i) != null).forEach(i -> setItem(i, button));
	}
	...
	{
		return IntStream.range(0, getSize()).filter(i -> getSize() < 27 || i < 9 || i % 9 == 0 || (i - 8) % 9 == 0 || i > getSize() - 9).parallel().toArray();
	}
	...
	public ConcurrentMap<Integer, GuiButton> getButtons()
	{
		return this.buttons;
	}
